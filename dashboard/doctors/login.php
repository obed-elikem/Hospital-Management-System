...
if (isset($_POST["login"])) {
    ...
    $sql = "SELECT id, password, status FROM doctors WHERE email = ?";
    ...
    if ($stmt) {
        ...
        if (mysqli_stmt_num_rows($stmt) > 0) {
            mysqli_stmt_bind_result($stmt, $doctors_id, $hashed_password, $account_status);
            mysqli_stmt_fetch($stmt);

            // Verify the password
            if (password_verify($password, $hashed_password)) {
                // Login successful, store the doctor's ID and status in the session
                $_SESSION["id"] = $doctors_id;
...
<body>
    ...
    <main class="container my-5">
        ...
        <h4 class="text-center text-blue mb-2">Doctors</h4>
