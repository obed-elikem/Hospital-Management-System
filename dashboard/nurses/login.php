...
if (isset($_POST["login"])) {
    ...
    // Fetch the Nurse's ID, hashed password, and status from the database based on the email
    $sql = "SELECT id, password, status FROM nurses WHERE email = ?";
    ...
    if ($stmt) {
        ...
        if (mysqli_stmt_num_rows($stmt) > 0) {
            mysqli_stmt_bind_result($stmt, $nurse_id, $hashed_password, $account_status);
            ...
            if (password_verify($password, $hashed_password)) {
                // Login successful, store the Nurse's ID and status in the session
                $_SESSION["id"] = $nurse_id;
                $_SESSION["status"] = $account_status;

                // Redirect to the Nurse's dashboard or other pages
            ...
            echo "<div class='alert alert-danger'>Nurse with the provided email not found.</div>";
        }
...
<body>
    ...
    <main class="container my-5">
        ...
        <h4 class="text-center text-blue mb-2">Nurses</h4>
