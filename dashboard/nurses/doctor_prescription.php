...
function getPatientIdByfullname($connection, $fullname)
{
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["search_prescriptions"])) {
    ...
    $patient_id = getPatientIdByfullname($connection, $fullname);

    if ($patient_id) {
        $prescriptions = fetchPrescriptionsByPatientId($connection, $patient_id);
    } else {
        // echo "Error: Patient with fullname $fullname not found.";
    }
}
?>
...
<body>
    <div class="container mt-5">
        ...
        <?php elseif (isset($_POST["search_prescriptions"]) && !isset($prescriptions)) : ?>
            <p class="text-danger">Error: Patient with name <?php echo $_POST["fullname"]; ?> not found.</p>
        <?php endif; ?>
    </div>
