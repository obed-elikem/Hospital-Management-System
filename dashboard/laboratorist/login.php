...
if (isset($_POST["login"])) {
    ...
    // Fetch the laboratorist's ID, hashed password, and status from the database based on the email
    $sql = "SELECT id, password, status FROM laboratorists WHERE email = ?";
    ...
    if ($stmt) {
        ...
        if (mysqli_stmt_num_rows($stmt) > 0) {
            mysqli_stmt_bind_result($stmt, $laboratorists_id, $hashed_password, $account_status);
            mysqli_stmt_fetch($stmt);

            // Verify the password
            if (password_verify($password, $hashed_password)) {
                // Login successful, store the laboratorist's ID and status in the session
                $_SESSION["id"] = $laboratorists_id;
                $_SESSION["status"] = $account_status;

                // Redirect to the laboratorist's dashboard or other pages
            ...
            echo "<div class='alert alert-danger'>laboratorist with the provided email not found.</div>";
        }
...
<body>
    ...
    <main class="container my-5">
        ...
        <h4 class="text-center text-blue mb-2">Laboratorists</h4>
