...
if (isset($_POST["login"])) {
    ...
    // Fetch the pharmacist's ID, hashed password, and status from the database based on the email
    $sql = "SELECT id, password, status FROM pharmacists WHERE email = ?";
    ...
    if ($stmt) {
        ...
        if (mysqli_stmt_num_rows($stmt) > 0) {
            mysqli_stmt_bind_result($stmt, $pharmacist_id, $hashed_password, $account_status);
            mysqli_stmt_fetch($stmt);

            // Verify the password
            if (password_verify($password, $hashed_password)) {
                // Login successful, store the pharmacist's ID and status in the session
                $_SESSION["id"] = $pharmacist_id;
                $_SESSION["status"] = $account_status;

                // Redirect to the pharmacist's dashboard or other pages
            ...
            echo "<div class='alert alert-danger'>Pharmacist with the provided email not found.</div>";
        }
...
<body>
    ...
    <main class="container my-5">
        ...
        <h4 class="text-center text-blue mb-2">Pharmacists</h4>
